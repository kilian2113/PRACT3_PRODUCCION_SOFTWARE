...
def test_negative_amount_raises_error():
    ...
    with pytest.raises(InvalidAmountError):
        Expense(id=1, title="Transporte", amount=-5, description="", expense_date=date.today())
...
def test_future_date_raises_error():
    ...
    with pytest.raises(InvalidExpenseDateError):
        Expense(id=1, title="Ocio", amount=20, description="Cine", expense_date=date.today() + timedelta(days=1))
